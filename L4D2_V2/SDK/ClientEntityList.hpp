...
{
public:
	virtual void LevelShutdown() { m_pClassList = 0; }
...
//
// 2. It provides a place to store IClientUnknowns and gives out int's
...
//	  given int's, and the handlers for spatial partition callbacks can
...
public:
	...
	virtual 					~CClientEntityList(void) = 0;
...
public:
	...
	int* GetClientEntity(int entnum); // index = 3

	virtual int					NumberOfEntities(bool bIncludeNonNetworkable = false); // index = 4

	virtual int* GetClientUnknownFromHandle(int hEnt); // index = 5
	virtual IClientNetworkable* GetClientNetworkableFromHandle(int hEnt); // index = 6
	IClientEntity* GetClientEntityFromHandle(int hEnt); // index = 7

	int	GetHighestEntityIndex(void); // index = 8

	virtual void				SetMaxEntities(int maxents); // index = 9
	virtual int					GetMaxEntities(); // index = 10


	// CBaseEntityList overrides.
protected:

	virtual void OnAddEntity(int* pEnt, int handle);
	virtual void OnRemoveEntity(int* pEnt, int handle);
...
public:
	...
	int* GetCollideable(int entnum);

	int* GetClientRenderableFromHandle(int hEnt);
	C_BaseEntity* GetBaseEntityFromHandle(int hEnt);
	int* GetCollideableFromHandle(int hEnt);
	int* GetClientThinkableFromHandle(int hEnt);

	// Convenience methods to convert between entindex + int
	int	EntIndexToHandle(int entnum);
	int						HandleToEntIndex(int handle);
...
// Inline methods
//-----------------------------------------------------------------------------
//inline bool	CClientEntityList::IsHandleValid(int handle) const
//{
//	return handle.Get() != 0;
//}
//
...
//}
//
//inline IClientUnknown* CClientEntityList::GetClientUnknownFromHandle(int hEnt)
//{
...
//// Convenience methods to convert between entindex + int
////-----------------------------------------------------------------------------
//inline int CClientEntityList::EntIndexToHandle(int entnum)
//{
//	if (entnum < -1)
